...
//        midiCallbackDevices.add (0);
		...
		// hook up with ALL the devices we want
        for (int i = enabledMidiInputs.size(); --i >= 0;)
        {
			{
                                                  ...
                                                  MidiInputCallback* /*callback*/)
{
    ...
    for (int i = midiCallbacks.size(); --i >= 0;)
    {
		...
		// if empty device, then remove all callbacks, as we added em all in addMidi .. above
		if (!name.isEmpty() && midiCallbackDevices.getUnchecked(i) != 0)
            devName = midiCallbackDevices.getUnchecked(i)->getName();

        if (name.isEmpty() || devName == name)
        {
...
void AudioDeviceManager::handleIncomingMidiMessageInt (MidiInput* source,
                                                       const MidiMessage& constMessage)
{
   MidiMessage message = constMessage;
    if (! message.isActiveSense())
    {
        const bool isDefaultSource = (source == 0 || source == enabledMidiInputs.getFirst());
        
        // stamp the incoming stuff with a midi channel so we can distinguish devices
        int deviceNumberToBecomeChannel = 0;
         int tmp;
         if (source != 0)
         {
            deviceNumberToBecomeChannel = enabledMidiInputs.indexOf(source) + 1;
            if (deviceNumberToBecomeChannel > 0 && deviceNumberToBecomeChannel <= 16)
               message.setChannel(deviceNumberToBecomeChannel);
         }
