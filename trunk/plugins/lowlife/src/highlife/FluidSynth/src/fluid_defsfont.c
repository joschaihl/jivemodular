...
int
fluid_defsfont_load_sampledata_into(fluid_defsfont_t* sfont,
                                    const int sourceStart,
                                    const int sourceSamples,
                                    float** destData,
                                    const int destStart,
                                    const int destSamples,
                                    const int destOffset)
{
  ...
  if (FLUID_FSEEK(fd, sfont->samplepos + sourceStart, SEEK_SET) == -1) {
    perror("error");
    FLUID_LOG(FLUID_ERR, "Failed to seek position in data file");
    return FLUID_FAILED;
  }
  ...
  unsigned int x;
  for (x = 0; x < sourceSamples; x++)
  {
      printf ("reading sample");

      short i_sample = 0;
      FLUID_FREAD (&i_sample, 1, sfont->samplesize, fd);
      
      if (((char *) &endian)[0]) {
        unsigned int i, j;
        unsigned char* cbuf = (unsigned char*) i_sample;
        for (i = 0, j = 0; j < sfont->samplesize; i++) {
	  ...
	  destData[0][destStart + x + destOffset] = (float) i_sample / 32768.0f;
  }
