      ...
      transformBox(0)
{
   ...
   addAndMakeVisible(transformBox = new ComboBox("FilterTransfor"));
   transformBox->addItem("Keep all", 1);
   transformBox->addItem("Keep notes", 2);
   transformBox->addItem("Keep CCs", 3);
...
void MidiFilterEditor::resized()
{
    ...
    transformBox->setBounds(16, 97, 200, 40);
...
void MidiFilterEditor::comboBoxChanged (ComboBox* comboBoxThatHasChanged)
{
    if (comboBoxThatHasChanged == transformBox)
    {
      MidiTransform* tran = owner->getMidiTransform();
      MidiTransform::TransformCommand cmd = MidiTransform::KeepEvents;
      switch (transformBox->getSelectedId()) 
      {
         case 2:
            cmd = MidiTransform::KeepNotes;
            break;
         case 3:
            cmd = MidiTransform::KeepCCs;
            break;
      }
      if (tran)
         tran->setTransformCommand(cmd);
    }
}
...
void MidiFilterEditor::updateParameters ()
{
    ...
    MidiTransform::TransformCommand cmd = MidiTransform::KeepEvents;
    MidiTransform* tr = owner->getMidiTransform();
    if (tr)
      cmd = tr->getTransformCommand();
   switch (cmd) {
      case MidiTransform::KeepNotes:
         transformBox->setSelectedId(2, true);
         break;
      case MidiTransform::KeepCCs:
         transformBox->setSelectedId(3, true);
         break;
      default:
         transformBox->setSelectedId(1, true);
         break;
   }
}
